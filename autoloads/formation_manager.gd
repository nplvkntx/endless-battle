...
func issue_formation_ground_order(
	...
	for fid: Variant in by_formation.keys():
		...
		var squad_result: Dictionary = PlayerRouteNavigation.issue_player_group_command(
			members,
			destination,
			order_kind,
			queued
	...
	# Unformed military in the same selection: PlayerRouteNavigation, else spacing.
