...
static func get_xp_amount_for_victim(victim: Node) -> int:
	...
	if victim is Spearman or victim is Swordsman or victim is Archer:
		if CombatTargetValidation.is_enemy_faction(victim):
			return MILITARY_XP
		return 0
...
static func _is_enemy_army_victim(victim: Node) -> bool:
	...
	if victim is Worker or victim is Spearman or victim is Swordsman or victim is Archer or victim is Hero:
...
static func _is_player_controlled_unit(node: Node) -> bool:
	...
	if node is Spearman or node is Swordsman or node is Archer or node is Worker:
