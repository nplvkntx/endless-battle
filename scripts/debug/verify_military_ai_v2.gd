...
func _verify_watchdog_config_and_source(failures: PackedStringArray) -> void:
	...
	if overlay_source != null:
		...
		_expect(failures, "F3 shows Workers", overlay_text.contains("Workers:"))
		_expect(failures, "F3 shows Camps", overlay_text.contains("Camps:"))
		_expect(failures, "F3 shows Tier", overlay_text.contains("Tier:"))
		_expect(failures, "F3 shows Expansion", overlay_text.contains("Expansion:"))
